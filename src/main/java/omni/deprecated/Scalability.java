        ...
        for (int i = 0; i < conditions.length; i++) {
            ...
            if (Main.rangeQueries) {
                if (Main.datasetName.equals("SNMP")) {
                    ///int[] order = {9, 14, 21, 19, 18, 1, 16, 12, 15, 5, 20,11, 17, 8, 13, 10};
                    int[] order = {12, 14, 11, 1, 5, 13, 9, 8, 10, 21, 15};
                    numAttrsToUse = order.length;
                }
            }
            this.rtp = new RamToPar(numAttrsToUse, Main.eps, Main.delta);
...
//       d.attributesInWorkload = ew.attributesUsed;
    ...
    public void readDatasetSettings() {
        switch (Main.datasetName) {
            case "CAIDA" -> {
                this.conditions = new String[]{"20"}; //"1", "5", "10", "15", //, "10", "15", "20"//Integer.toString(Main.filesToRead)};//, "4", "5", "6", "7", "8", "9", "10", "11", "12"};
