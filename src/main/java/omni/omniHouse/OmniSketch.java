...
//    CountMinDyad[][] CMSketchesRange;
    ...
    public OmniSketch(long ram, int numStoredAttributes, int[] parameters, int dyadicBits,
                      ...
                      double BetaKmin, boolean dynamicSampleSizes, boolean case1ReturnScap, int seed) {
    ...
    public void initSketch() {
        setting = "OmniSketchHouse_" + dynamicSampleSizes;
        Main.kminDeletes = 0;
//        eps = 1/(Math.pow(((width -1)/Math.exp(1)),depth) - 1);
        if (useAcrossRows) {
            eps = 0.1;//1 / (Math.pow(((width - 1) / Math.exp(1)), depth));
        } else {
            eps = 0.1;//Math.exp(1)/(width -1);
        }
    ...
    private int getAltEstKMV(PriorityQueue[] samples) {
        ...
        for (int i = 0; i < numJoins; i++) {
            if (!samples[i].isEmpty()) {
                currentValues[i] = (int) samples[i].peek();
            } else {
