...
public class AttrSketchHashSetNaive {
    ...
    public AttrSketchHashSetNaive(OmniSketchConfig sketchConfig, int depth, int width, int sampleSize) {
    ...
    public void ingest(long attrValue, long sampleToReplace, int sign) {
        hash(attrValue);
        for (int j = 0; j < depth; j++) {
            int w = reusableHashes[j];
            if (sign == 1) {
                sketch[j][w].add((int) sampleToReplace);
            } else {
                sketch[j][w].remove((int) sampleToReplace);
            }
        }
    }
