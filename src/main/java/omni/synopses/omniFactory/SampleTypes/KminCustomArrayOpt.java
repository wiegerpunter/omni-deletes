...
public class KminCustomArrayOpt implements Sample {

    private final int B;
    private final int ingestionBufferSize;
    private final int b;
    private int n;
    private ArrayWithBufferOptimized sample;
    ...
    public KminCustomArrayOpt(int B, int b, double beta, int ingestBufferSize, String setting, boolean onlyUseValidSamples, boolean pessimisticDeleteCounter) {
        this.b = b;
        ...
        ingestionBufferSize = ingestBufferSize;//(int) Math.min(150,Math.max(1, (B*beta) / 80)); // todo: optimize buffer size of ingestion.
        if (beta == 0) {
        ...
        this.sample = new ArrayWithBufferOptimized(this.B, ingestionBufferSize);
    }
...
//        analyzeDeletes();
    ...
    @Override
    public void reset() {
        sample = new ArrayWithBufferOptimized(B, this.ingestionBufferSize); // Resetting the sample without a buffer
        n = 0;
    }
