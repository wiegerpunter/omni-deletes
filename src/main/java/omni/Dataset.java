    ...
    public Dataset(int numQueries, int range, String setting, int numAttrsToUse, boolean[] attrsToRead, Helper h) throws IOException, CsvValidationException {
        ...
        switch (setting) {
            case "SNMP" -> {
                ...
                //Main.numAttributes = dataset.get(0).record.size();
            }
            case "wc98" -> {
                Main.numAttributes = numAttrsToUse;

                // Check if file exists
                String CSV_FILE_NAME = Main.outputFolder + "WCDataset_" + Main.datasetName + "_"+ Main.fileStartCondition + ".csv";
                File f = new File(CSV_FILE_NAME);
...
//        if (Main.filesToRead != 1) {
//            // Recompute q.exactAnswer
//            long[] rec;
//            for (Query q: queries) {
//                q.exactAnswer = 0;
//            }
...
//        }
    ...
    private void loadFileWC(String path) {
        ...
        if (file.getName().endsWith(".gz")) {
            ...
            ProcessedStreamLoaderGeneric psl = new ProcessedStreamLoaderGeneric(file.getPath(), true);
            psl.reset();
            //psl.readHeader();
    ...
    private void rangeQueryWorkload(int attr, StringBuilder queryString, Boolean[] seenAttributes) {
//        StringBuilder prevQueryString = new StringBuilder(queryString);
//        if (!seenAttributes[attr]) {
//            seenAttributes[attr] = true;
//            AttributeStats a = dStats[attr];
//            if (!a.categorical) {
//                int minQueryRange = (int) (a.avg);
//                int maxQueryRange = (int) (a.avg + 0.1 * a.stdDev);
//                if (!a.sanityCheckRange(minQueryRange, maxQueryRange)) {
//                    return;
//                }
//                if (prevQueryString.toString().equals("")) {
//                    prevQueryString.append(a.attributeName).append(" in ").append(minQueryRange).append(",").append(maxQueryRange);
//                } else {
//                    prevQueryString.append(" and ").append(a.attributeName).append(" in ").append(minQueryRange).append(",").append(maxQueryRange);
//                }
//                Query q = new Parser(queryId).parse(prevQueryString.toString());
//                queryId++;
//                if (exactSolution(q) <= 0) {
//                    return;
//                }
//                queries.add(q);
//
//                if (Arrays.asList(seenAttributes).contains(false)) {
//                    for (int i = 0; i < Main.numAttributes; i++) {
//                        if (!seenAttributes[i]) {
//                            rangeQueryWorkload(i, prevQueryString, seenAttributes);
//                            seenAttributes[i] = false;
//                        }
//                    }
//                }
//            }
//        }
    ...
    public void AltWorkload() throws IOException {
...
//    public static Integer getRandomKey(LinkedHashMap<Integer, ArrayList<Integer>> linkedHashMap, int seed) {
...
//        return (Integer) keyArray[randomIndex];
//    }
...
//    public void synthDataset(int numRecords, int numZipfianAttributes, double zipfAlpha) throws IOException {
//        h.initSynthDataset();
//
...
//        //zipfianSumArray = generateZipfianSumArray(numRecords, zipfAlpha);
    ...
    public static long generateZipfianValue(double alpha, int numValues, Random random, long[] zipfianSumArray) {
        double u = random.nextDouble();
        double proportionalValue = 0.0;

        for (int i = 1; i <= numValues; i++) {
            proportionalValue += (1.0 / Math.pow(i, alpha)) / zipfianSumArray[numValues - 1];

            if (u <= proportionalValue) {
                return i - 1;
            }
        }
    ...
    public static long[] generateZipfianSumArray(int numValues, double alpha) {
        long[] zipfianSumArray = new long[numValues];
        double sum = 0.0;

        for (int i = 1; i <= numValues; i++) {
            sum += 1.0 / Math.pow(i, alpha);
            zipfianSumArray[i - 1] = (long) sum;
        }

        return zipfianSumArray;
    }
...
/*
    ...
    public void syntheticDataset(int size, int range) {
        //Random rn = new Random(seed);
        for (int i = 0; i < size; i++) {
            Random rn = new Random((long) i);
            ArrayList<Integer> record = new ArrayList<Integer>(numAttributes);
            for (int j = 0; j < numAttributes; j++) {
                //record.add(0);
                record.add(rn.nextInt(range));
            }
            dataset.add(new Record(i, record));
        }
    }
