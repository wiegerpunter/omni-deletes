    ...
    public void initSketch() {
        ...
        if (!rangeQueries) {
            ...
            for (int i = 0; i < numStoredAttributes; i++) {
                CMSketches[i] = new CountMin(i, parameters, useTwoLHS, useTwoKmin, twoLHSFast);
            }
            ...
            for (int i = 0; i < numStoredAttributes; i++) {
                for (int j = dyadicRangeBits; j >-1; j--) {
                    CMSketchesRange[i][dyadicRangeBits - j] = new CountMinDyad(i, j, dyadicRangeBits, parameters, useTwoLHS);
                }
            }
...
//                / Main.delta)/(Main.eps * Main.eps);
    ...
    public int query(long[] q, int numPreds, int unionSize, AnalysisBaselinesRefactor.QueryInfo CMRow) {
        if (Main.estPerRow) {
            ...
            return queryEstOverAllRows(q, numPreds, unionSize, CMRow);
        }
    }
